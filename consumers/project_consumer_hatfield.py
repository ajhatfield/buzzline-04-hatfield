...
def update_chart():
    """Update the live chart with the latest topic counts."""
    ...
    ax.bar(topic_list, counts_list, color="red")

    # Use the built-in axes methods to set the labels and title
    ax.set_xlabel("Topic")
    ax.set_ylabel("Message Counts")
    ax.set_title("Real-Time Topic Message Counts")
...
def process_message(message: str) -> None:
    ...
    try:
        ...
        if isinstance(message_dict, dict):
            ...
            logger.info(f"Updated topic counts: {dict(topic_counts)}")
